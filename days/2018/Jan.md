...
* *單元測試的藝術*：接著昨天的話題，來談談如何讓測試好維護：
    * 只測試公開方法，不要去測私有方法；如果你真的想測私有方法，那麼或許讓它成為一個公開方法是個好方法(繞口令)。
    * 避免過度指定結果，否則即使程式碼的改動符合需求，也可能因為過度指定而導致測試失敗。例如，如果字串只需要**包含**另一個字串就好的話，就使用`String.contains`而不要使用`String.equal`。
    * 不要用`setup`，這會讓測試程式碼不易看出執行的上下文。如果要用，也最好只用在所有測試都會需要的初始化動作，而不是只針對特定幾個測試。
    * 測試的環境要獨立不可互相依賴。共享的資源/狀態在每一個測試結束後應該要回到初始值；測試不可以呼叫另一個測試；測試不可以依賴於呼叫的順序。這些都可以確保一旦測試失敗，就只和測試程式碼本身有關係，而不會是任何外部因素。
    * 減少重複的程式碼，例如前述的工廠方法，或者把常用的操作包成小函數。但也不能做得太過頭，抽象過度也會使程式碼變得不好讀。
