...
* 還是*Perfect Software: And Other illusions about testing*：

    * 狗食測試(dogfooding)：讓開發人員使用自己開發出來的東西，有助於增加對產品的信心。從公司的角度來看，也可以指整間公司都使用自家的產品。[wiki](https://zh.wikipedia.org/wiki/Eating_your_own_dog_food)
    * 覆蓋範圍測試可以讓你知道系統的哪些部分還沒有被測試過。100%的測試覆蓋率不能說明程式碼有良好的測試，但低的測試覆蓋率可以說明測試很糟糕。
    * 用防衛機轉來解釋專案中各種面對bug的反應(忽視缺陷、合理化缺陷(feature即是bug!)、轉移、投射、過度反應...)真的是很有趣呢。想維持完美、不想被指出有錯、不想承認自己犯蠢、不想趕不上進度、不想要因為指出事實而挨罵、不想要找出問題、不想要放過問題......我也曾因為這些恐懼而選擇對眼前的錯誤視而不見，最後反而把事情做得很糟。放輕鬆啊，放輕鬆(欸)

## 01/20, Sat.

### 技術
* [Secure Contexts Everywhere](https://blog.mozilla.org/security/2018/01/15/secure-contexts-everywhere/)：Firefox表示將來不是https不能用新的feature啦。Chrome先前也做了類似的宣告，但一直沒有像Firefox在這篇文章裡說的這麼肯定。總之，http在這幾年應該會漸漸消失(吧)

## 01/21, Sun.

### 技術

* [Test Pyramid](https://martinfowler.com/bliki/TestPyramid.html)：指一個專案的測試可以大致分成三種，依照數量呈金字塔型：

    * 最下層的是單元測試，關注的範圍只是系統中的一個模塊，通常運行的速度很快，由開發者自行編寫，數量也會是最多的。
    * 最上層的則是end-to-end test(e2e test)，泛指直接透過UI對系統的多個部分是否能一起正確運作進行測試。通常運行得比較慢，不方便撰寫，而且較不穩定，因此一個專案不應該有太多的e2e test。
    * 中間則是撇開UI而直接對系統的多個部分作整合測試(Martin Folwer稱之為Subcutaneous Test)，這些測試不需要處理UI層，因此運行起來會比較快，但又可以測試系統中的多個部件是否能一起運作。

    Test Pyramid即是主張一個專案應該有大量的單元測試、少量的e2e測試，及中間數量的Subcutaneous Test。過多的e2e test會使跑測試的速度變慢，讓人不願意常常做測試，而且也容易因為需求變更而需要修復。

### 錢錢

* 一直都弄不清楚怎麼跟它打交道，在圖書館找了幾本有趣的書：

    * [愛錢](https://www.taaze.tw/usedList.html?oid=11100633936)：並不是談及如何理財，而是讓你理清自己的金錢觀的書。
    * [所有的人都該有錢](http://www.eslite.com/product.aspx?pgid=1001129721812914&kw=%E6%89%80%E6%9C%89%E7%9A%84%E4%BA%BA%E9%83%BD%E8%A9%B2%E6%9C%89%E9%8C%A2&pi=0)：介紹一些簡單的概念，但對於新手如我似乎蠻有用的。
...
* 依舊是*Perfect Software: And Other illusions about testing*：

    * 缺陷的重要性是一件主觀的、帶有情緒的事情，不同的角色(PM、開發者、測試者、行銷、老闆...)對一個缺陷有多重要的看法是不同的，因此我們能做的是去考量各個方面的重要性，盡量減少(但不可避免)情緒的影響，去處理那些比較重要的缺陷。
    * 解決缺陷的過程很單純：**FFF**：發現(find)、評估(figure)、修復(fix)
    * 一些人會把專案的失敗推託給運氣不好：地震、風災、團隊裡面的重要人員出車禍，等等等。然而事實上，也有許多即使碰到運氣不好，仍然取得成功的專案，差別在於他們做好管理：技術評審、寫文件、做好測試、等等等，使得他們在壞運氣到來時有備無患。
    * 如果到了專案的尾聲還有修不完的缺陷，應該怎麼做呢？

        1. 停止所有的測試(既然bug已經夠多了，再花時間找bug只是減少修復已知bug的時間；即使要找，也應該先記錄下來，留待之後再修復)
        2. 依照重要性對缺陷排序
        3. 估算在剩餘的時間依照上面的順序可以修復多少bug
        4. 在交付計畫中去掉在3中無法去掉的功能；如果這個功能重要到不能被去掉，取消交付。
        5. debug

    * 為什麼對測試時間的估計和現實差距這麼大？

        * 以為天天都過年，專案進度都會一路暢通。
        * 忽略了測試、確定重要性和debug所需要的大量時間。
        * 承上，即使考慮了，用以往專案修復缺陷的過程數據(缺陷產生率、修復時間、故障反饋率)來計算，也往往低估；一件有趣的事實是，一個專案的故障反饋率(Fault Feedback Rate(FFR)，指修復了一個缺陷之後產生另一個缺陷的機率)往往被人們低估，或甚至不考慮，事實上，如果兩個bug中有一個修好之後沒有任何因此造成的新bug(FFR = 50%)，不少團隊就會很開心了；你可以試著算算看，在FFR = 0和FFR = 50%的狀況下，所需要的缺陷修復時間差多少。
        * 承上，更可能的是根本沒有任何過程數據的記錄，以致於上述的估算根本不準。
    * 通常如果到了測試的時候才發現時程來不及，就已經太晚了；除了重新開始之外很可能沒有其它方法可以解決。因此時程的管理很重要。
...
* *單元測試的藝術*：優秀的測試程式碼包含三個要素：
    * 可讀：看得懂
    * 可維護：不會改到很煩
    * 可靠：跑一百遍結果都一樣正確而且你相信它一定是對的。
    
    之後再一一介紹為了達成這三件事情需要做到的事。

* [Design a Perfect Search Box](https://uxplanet.org/design-a-perfect-search-box-b6baaf9599c)：列出了設計搜尋列的幾個要點：
    * 請使用放大鏡icon
    * 如果搜尋是網站的重要功能，請讓它明顯一些，最好整條搜尋列都放上去而不是只有放大鏡icon
    * 提供明顯的搜尋按鈕，讓搜尋列可以透過enter鍵和按鈕做搜尋。不少人還是習慣用按按鈕的方式觸發搜尋。
    * 把搜尋列放在每一頁，這樣人家迷路才方便用搜尋找路。
    * 不要直接用一堆選項嚇跑用戶，愈簡單愈好。
    * 請把它放在用戶直覺認為搜尋列**應該**會在的地方，根據研究，右上角和左上角乃至於整個畫面上方，是用戶期待能找到搜尋列的地方。
    * 請讓輸入欄位足夠長，太短用戶看不到他輸入的內容，一般而言，27個英文字長可以包含90%的查詢內容。
    * 請妥善使用auto suggestion
        * 確保它有用，例如拼字校正。
        * 盡快顯示出來，通常在輸入二到三個字元時就可以顯示了
        * 不要放太多，盡量小於10個項目
        * 可以用不同的字重/字體來區別輸入的內容和建議項目
        * 允許用戶用上下方向鍵切換到建議的項目。
    * 適度的用placeholder提示搜尋列可以找的東西

## 01/26, Fri.

### 技術

* [Is unit testing worth the effort?](https://stackoverflow.com/questions/67299/is-unit-testing-worth-the-effort)：坦白說，現在工作上的專案通常趕完功能都來不及了，都沒有寫unit test。有兩種矛盾的情緒：
    * 不寫嘛，但是總是會擔心自己改的東西到底會不會有問題，雖然用手測過了，但搞不好有我漏掉的東西，唉呀呀...雖然大多數時候是沒事啦，但是總是有點不安心，而且，是**大多數時候**。
    * 寫嘛，可是案子本來按不寫測試的速度做，就已經有一些些吃緊了，大概是不趕工可以剛好完成功能的速度，寫下去，不知道要多花多少時間，也不知道它可以為我省下多少時間。怕的是寫的測試最後都沒派上用場，然後又讓案子變得很趕。
    這篇SO裡面有一則回答倒是很有趣(稍微經過加油添醋(欸))：
        * 寫單元測試就像上健身房一樣，你聽很多人說它對身體很好，而且是有實證根據的很好。所以你就開始上健身房。
        * 剛開始很新鮮也很快樂，但過了一開始的熱潮之後，你開始懷疑上健身房到底有沒有用，但你總是每週花上幾個小時去運動。
        * 然後某一天，工作忽然出現了一個無論如何都得趕上的死線，於是你拚了命的趕死線，加班、把你所有良好的生活習慣通通都拋棄，當然，也包括上健身房。
        * 好不容易你終於趕上它了，回頭看，到底當初為什麼要健身呢？它看起來對你的生活好像沒什麼幫助，你開始懷疑自己，會不會是自己搞錯了健身的方式，或者其實根本健身就是不必要的。你也開始回想，健身對你而言也是挺麻煩的––要回家換衣服、移動到健身房、繳錢給櫃台、還可能會被家中長輩問「幹嘛要花錢去健身啊？自己在家裡做伏地挺身就好啦」花錢花時間也花精力。
        * 你發現那些宣揚健身的人，他們好像不會像你那麼麻煩––他們的工作場所就有健身房、他們把健身當成理所當然、像吃飯一樣的事、他們也不會因為忽然有一個死線從天上掉下來就因此被老問告知「健身不重要啦，快去趕工。」。
    * 回覆者想表達的是：
        * 單元測試大致上有用，但是每個人所需付出的努力有很大的不同。而環境對此影響很大。專案時程、程式碼的乾淨程度、團隊文化、老闆都會影響你做單元測試的困難程度。
        * 也許你該做的是，去到另一個會把單元測試當成理所當然的地方，這樣就會輕鬆多了。或者，把你所在的地方，打造成另一個可以輕鬆實行單元測試的地方。
...
明天開始會改成早上更新；果然沒辦法當夜貓子啊。

## 01/30, Tue.

### 技術

* *單元測試的藝術*：接著上一次的話題，來談談寫測試的一些要點：
    * 怎麼樣才能讓測試的結果可靠呢？
        * 一次只測一個關注點。如果在一個測試中同時測了兩件事，斷言失敗的時候，我們無從知道是哪一件事失敗了，因此變得不好除錯。具體來說，如果一個測試中有兩個以上的斷言，而你即使在其中一個斷言失敗的狀況下，仍然會想要知道另一個斷言的結果，那就應該為這個斷言另外寫一個測試。
        * 刪除或修改不適當的測試。可以從測試失敗的原因來思考：
            * 程式碼缺陷：測試抓到了程式碼的bug，這是測試所期待的結果，所以不用修改測試。
            * 測試缺陷：測試本身有bug，這樣我們就應該把測試的bug修好。有三個步驟：
                1. 修測試缺陷
                2. 確保測試該失敗的時候失敗
                3. 確保測試該通過時通過。

                開發者往往會急於修復測試而只做了1和3，看到測試過了就不管它了，卻可能不小心把斷言語句刪掉了，導致錯的東西仍然能夠通過測試，因此2這一步很重要。
            * 程式碼語義或API變更：被測試的程式碼用法變了，導致使用它的測試壞掉。在此，為了減少修改的困難和工作量，可以把常用的操作或初始化用一個函數包起來，如此當API變化的時候，只要改一個地方就可以了。
            * 新加入的功能/測試和舊測試衝突：代表產品的新功能會使舊功能失敗，此時應該確認需求為何，決定要保留哪個測試。
            * 重覆的測試：當然測試是愈多愈好，然而重覆的測試也可能使維護變得困難，因此通常還是建議去掉。
        * 避免測試中的邏輯語句：例如`if-else`、`while`甚至是 `try-catch`，這會讓它變得不好讀，而且容易出錯。
        * 避免動態定義斷言值，使用硬編碼：例如

        ```javascript        
        test("Build greeting string", () => {
          const builder = new StringBuilder()
          const user = "USER"
          const greeting = "GREETING"
          expect(builder.build(user, greeting)).toBe(user + " " + greeting)  
        })
        ```
        就不如
        ...
        test("Build greeting string", () => {
          ...
          const result = builder.build(user, greeting)
        ...
        *  把單元測試和集成測試分開。
        *  測試覆蓋率和code review：測量測試覆蓋率如前所述，是為了確保它不會太低，太低的測試覆蓋率代表有很多程式碼缺少測試；高的測試覆蓋率只能代表大多數的程式碼都有測試，但不代表有**好好的**被測試，因此還是需要有人做code review來看測試是否有效。
